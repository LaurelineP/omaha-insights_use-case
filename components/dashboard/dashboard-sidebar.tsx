...
import { ChartBarIcon, ChartSplineIcon, InfoIcon, LayoutPanelTopIcon } from "lucide-react";
  ...
  return (
    <Sidebar className="bg-sidebar-primary shadow-xl ">
        ...
        <SidebarHeader className = "px-4 pt-6 leading-4">
          <h1 className    = "text-2xl font-semibold text-indigo-800">Metric Views</h1>

          {/* View Description */}
          <div className   = "flex flex-col font-normal text-gray-500 p-2">
            <div className = {
                cn( "flex items-center font-normal text-xxl text-gray-500 text-sm gap-2 rounded p-1.5 justify-between border border-transparent",
                  didClickOnDisabled && "bg-warning/10 text-warning border boder-warning/20 animate-shake",
                )}>
              <p className = "wrap-break-word w-fit">
                Pick <b>exactly 3 metrics</b> to<br/> display in the dashboard charts.
              </p>
          ...
          {checkboxesStructure?.map(([group, options], idx) => {
            const description = metricDescription?.[group as MetricType]
              ...
              .map((text: string, idx: number) => (
                <span
                  key = {`${group}-${idx}`}
                  className={cn(
                    "flex block my-2", idx === 0 && "flex-stretch"
                  )}>
                  {text}
                </span>
              ));
            return (
              ...
              <SidebarGroup key = { idx } className="w-full pb-10">
                <FieldSet className ="ml-2.5 my-0 gap-0" onClick = { handleCheckboxChanges }>
                  ...
                  <FieldGroup className="text-sm gap-2 pl-4 mt-3">
                    {options.map((option: { fieldKey: Exclude<ChartViewOption, ''>; metric: string; type: string }, idx: number) => {
                      const isDisabled = (selectedCharts.filter(Boolean).length === 3 && !selectedCharts.includes(option.fieldKey))
                      const isSelelected = selectedCharts?.includes( option.fieldKey )
                      const selectedIndex = selectedCharts?.indexOf(option.fieldKey)
                      return (
                        ...
                        >
                          <div className   = "flex w-full justify-between h-5">
                            <span className="inline">{ metricTextMap[ option.fieldKey ].chartTooltip }</span>
                            {
                              isSelelected && (
                                <span className="inline rounded-2xl border px-2 py-1 uppercase text-[.5rem] text-gray-600">
                                  {selectedIndex === 0 && 'top'}
